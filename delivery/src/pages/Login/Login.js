...
const Login = () => {
  const { isAuthed, setSession } = useContext(AuthContext);
  ...
  const sendOtp = async (e) => {
    ...
    setSubmitting(true);
    try {
      await api.post(`${R}/auth/send-otp`, { phone });
        ...
        {step === "phone" ? (
          ...
          <form onSubmit={verifyOtp} className="al-form">
            ...
            <div className="al-login-foot">
              <button type="button" className="al-link-btn" onClick={() => setStep("phone")}>
                ← Change number
              </button>
              <button type="button" className="al-link-btn" onClick={sendOtp} disabled={submitting}>
                Resend OTP
              </button>
            </div>
          </form>
